...
Two helpers, split by who owns the connection:

* :func:`get_queue` — wraps the asyncpg connection underlying a
  SQLAlchemy ``AsyncSession`` so jobs enqueue inside the *same*
  transaction as the request's other writes.  The job becomes
  durable when the session commits; if the session rolls back,
  the job is gone.  This is the transactional-outbox pattern.

* :func:`open_worker_driver` — opens a dedicated asyncpg
  connection for the worker process.  Workers are long-lived and
  must own their connection (pgqueuer ``LISTEN``s on it for
  notifications), so they cannot share the request-scoped
  pool.
